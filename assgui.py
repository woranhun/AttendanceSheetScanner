...
class MainGUI(ASSGUI):
    ...
    def __init__(self, master, img):
        ...
        self.create_button("Táblázat bejelölése", 0, 0, self.set_area_click)
        self.create_button("PDF megnyitása", 3, 0, self.open_pdf)
        self.create_button("Kép kiválasztása", 4, 0, self.select_image)
        self.create_button("Aláírások kigyűjtése", 6, 0, self.detect_signatures)
        self.create_button("Mentés", 7, 0, self.save)
        self.create_button("Törlés", 8, 0, self.clear)
        FileMGMT.create_dirs()

    def save(self, _) -> None:
        FileMGMT.save_to_csv(self.students)

    def clear(self, _) -> None:
        self.students = []

    def set_area_click(self, _):
        root = tk.Toplevel()
    ...
    @staticmethod
    def open_pdf(_):
        FileMGMT.convert_pdf_to_img(filedialog.askopenfilename())
    ...
    def set_scan_area(self, scan_area: Quad) -> None:
        ...
        points = GraphicsMath.find_line_intersection(img)
...
class DetectSignatures(ASSGUI):
    ...
    def __init__(self, master, img: np.ndarray, grid: List[List[Quad]], students: List[Student], class_num: int):
        ...
        for i in range(len(grid[0])):
            ...
            print('"', name, '"', sep="")
            sign = ComputerVision.get_signature_from_area((nparray_to_point(sign_quad[0]), nparray_to_point(sign_quad[2])),
                                                          self.background_image)
        ...
        self.canvas.bind("<Button-1>", self.mouse_click_on_canvas)
        self.create_button("Cancel", 1, 0, self.cancel)
